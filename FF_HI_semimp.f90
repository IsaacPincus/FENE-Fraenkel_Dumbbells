...
!    Vpsi2(timestep) = sum(((tau(2,2,:) - tau(3,3,:)))**2)/sr**4
    Qavg(timestep) = sum(Q(1,:)**2 + Q(2,:)**2 + Q(3,:)**2)
    Vqavg(timestep) = sum(sqrt(Q(1,:)**2 + Q(2,:)**2 + Q(3,:)**2))
...
!10 format(F4.2,4X,F7.5,4X,F7.5,4X,F7.5,4X,F7.5,4X,F7.5,4X,F7.5,4X,F7.5,4X,F7.5)
!do i = 1,Ndtwidths
!    write(*,10) timestepwidths(i), Aeta(i), Veta(i), Apsi(i), Vpsi(i), &
...
pure function find_roots(a, b, c, Q0, lim)
    ...
    do i=-1,1
        ...
        if ((x.ge.(Q0)).and.(x.le.(Q0+lim))) then
            find_roots = x
        end if
   end do
